...
resource "terraform_data" "push_image" {
  depends_on = [aws_ecrpublic_repository.example_lambda_repo]
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      REPO_URL       = aws_ecrpublic_repository.example_lambda_repo.repository_uri
    }
  }
}
...
resource "aws_lambda_function" "example_lambda_repo_function" {
  ...
  package_type  = "Image"
  image_uri     = "${aws_ecrpublic_repository.example_lambda_repo.repository_uri}:latest"
